...
原有模块保留供兼容使用。搜索/下载新入口见 [主 README](../README.md)。下方协议示例不属于新下载模块的依赖，也未在本次迭代中做线上验证。
...
- **项目作者：** [jzjzzzzzzz](https://github.com/jzjzzzzzzz)
...
**一旦您下载本项目，即视为您已同意并接受上述所有条款。**



## 许可证 (License)

当前许可条款以仓库根目录的 [LICENSE](../LICENSE) 为准（专有许可）。
...
```bash
cd /Users/jz/RedCrack
python3 -m pip install -r requirements.txt
```

自动发布默认优先使用本机 Google Chrome；如果没有 Chrome，可安装 Playwright Chromium：
...
自动发布使用小红书官方创作服务平台和独立的持久化浏览器目录，不依赖游客
`web_session`。第一次使用时打开浏览器扫码登录：
...
```text
~/.redcrack/xhs_creator_profile
```

发布图文笔记：

```bash
python3 publish.py image \
  --title "今天的晚霞" \
  --content "下班路上遇到的晚霞。" \
  --tags 摄影 日常生活 \
  --images /绝对路径/cover.jpg /绝对路径/detail.jpg
```

发布视频笔记：

```bash
python3 publish.py video \
  --title "周末散步" \
  --content "记录一个轻松的周末。" \
  --tags 周末 vlog \
...
```bash
python3 publish.py image --headless \
  --title "标题" \
  --content-file /绝对路径/content.txt \
  --images /绝对路径/cover.jpg
```

使用 `--dry-run` 时只上传素材并填写表单，不点击发布按钮。
...
from request.creator import XHSCreatorPublisher


async def main():
    async with XHSCreatorPublisher(headless=True) as publisher:
...
发布模块兼容新版 `xhs-publish-btn` Web Component，支持：
